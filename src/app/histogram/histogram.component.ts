...
import { DataSets, DonneeTranche } from '../app.component.models';
@Component({
...
/*   private chargerDonneesTranche(tranche: string): Promise<any> {
  ...
  const dataStructure: [number, number][] =  data.results.map((item) => [
    new Date(item.date_et_heure_fuseau_horaire_europe_paris).getTime(),
